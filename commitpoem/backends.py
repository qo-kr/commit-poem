...
def _build_prompt(commits: list[str]) -> str:
    ...
    return (
        ...
        f"{commits_text}\n\n"
        "Read between the lines: sense what was made, mended, torn down, or wrestled with "
        "today — and the feeling behind the work (relief, persistence, a small triumph, "
        "a letting-go).\n\n"
        "Then SUBLIMATE the technical into the poetic. Never name functions, files, "
        "variables, flags, commands, or any code/jargon literally — and do not quote the "
        "commit messages. Transmute the work into image and metaphor instead:\n"
        "  - a bug fixed → a crack sealed, a knot loosened, a fever broken\n"
        ...
        "Keep the poem rooted in the concrete spirit of *this* day's work, yet let the "
        "machinery stay hidden beneath the image.\n\n"
        "Write TWO short poems (4-8 lines each):\n"
        "1. First poem: in English\n"
        "2. Second poem: in Korean (한국어) — 자연스러운 우리말 시로, 영어 단어나 기술 용어는 쓰지 말 것\n\n"
        f"Write in the distinctive style of {poet}. "
        f"End each poem with: \"— in the style of {poet}\"\n"
        "Label them with 🇺🇸 and 🇰🇷 headers.\n"
        "Be evocative and specific in feeling, never generic — but keep the code itself "
        "invisible, dissolved into the poem."
    )
